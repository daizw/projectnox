...
public class TestDHKey {
	...
	private void run() throws Exception {
		...
		/**
		 * bob从alice发来的公钥中提取Alice初始化其DH对时的初始化参数,
		 * 用以初始化bob自己的DH对.
		 */
		...
		// 李四(alice) 生成本地的密钥 aliceDesKey
		SecretKey aliceDesKey = aliceKeyAgree.generateSecret("DES");
		System.out.println("ALICE: 用bob的公钥定位本地key,并生成本地DES密钥");
		
		if (aliceDesKey.equals(bobDesKey))
			System.out.println("李四和张三的密钥相同");
		// 现在李四和张三的本地的deskey是相同的所以,完全可以进行发送加密,接收后解密,达到
		// 安全通道的的目的
		/**
		 * bob用bobDesKey密钥加密信息
		 */
		...
		String bobinfo = "这是张三的机密信息";
		System.out.println("张三加密前原文:" + bobinfo);
